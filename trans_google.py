...
#delete TOTAL country
		...
		numeric_cols = ['user data requests', 'percentage of requests where some data produced', 'users/accounts specified']
		...
		utils.df_convert_from_percentage(df, 'percentage of requests where some data produced', 'user data requests', 'number where some information produced')
	...
	def read_csv(self, filename_or_url):
		with ZipFile(filename_or_url) as zipf:
			with zipf.open('google-user-data-requests/google-user-data-requests.csv') as data_file:
				df = pd.read_csv(data_file, encoding="UTF-8", dtype=np.object_)  #force dtype to avoid columns changing type because sometimes they have *s in them
